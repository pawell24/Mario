...
export default class ImageManager implements IImageManager {
    ...
    */
    ...
    setDelete(elements: VisualElement[]) {
        const pills: PillChunk[] = elements.filter(x => (<PillChunk>x).id !== undefined).map(x => <PillChunk>x)
        const virus: VirusChunk[] = elements.filter(x => (<PillChunk>x).id === undefined).map(x => <VirusChunk>x)
